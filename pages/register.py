...
class RegisterPage:
    ...
    def get_selected_characters(self):
        ...
        try:
            response = self.supabase.table('users').select('character_name').execute()
            if response.data:
                return [user['character_name'] for user in response.data]
            return []
        except Exception as e:
    ...
    def save_to_db(self, data):
        ...
        try:
            # Prepare data dictionary for Supabase
            user_data = {
                'nombre': data[0],
                'email': data[1],
                'deseo1': data[2],
                'link_deseo1': data[3],
                'imagen_deseo1': data[4],
                'deseo2': data[5],
                'link_deseo2': data[6],
                'imagen_deseo2': data[7],
                'deseo3': data[8],
                'link_deseo3': data[9],
    ...
    def render_form(self):
        ...
        st.markdown("<h1 style='text-align: center; color: white; font-size: 2.5em;'>🎄 Registro 🎄</h1>",
                    unsafe_allow_html=True)
        ...
        with col2:
            ...
            available_characters = [char for char in all_characters
                                   if char['name'] not in selected_characters]
